...
class AdvancedPlanetGenerator:
    def __init__(self):
        ...
        self.planet_types = {
            ...
            for cls in pty.PlanetType.__subclasses__()
        }
    ...
    def apply_climate_effects(self, tile, pos, height_map, planet_type):
        ...
        if height > 0.7:
            if isinstance(planet_type, pty.GreenPlanet) and random.random() < 0.3:
                tile.landscape = pt.Snow()
            elif isinstance(planet_type, pty.DryPlanet) and random.random() < 0.2:
                tile.landscape = pt.Mountains()
        elif height < -0.3:
            if isinstance(planet_type, pty.VolcanicPlanet) and random.random() < 0.4:
